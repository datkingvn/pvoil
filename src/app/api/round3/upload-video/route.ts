...
const MAX_FILE_SIZE = 100 * 1024 * 1024; // 100MB cho video
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      return NextResponse.json(
        { error: "Chỉ chấp nhận file video: MP4, WebM, MOV, AVI" },
        { status: 400 }
      );
    }
    ...
    const filename = `round3/videos/${timestamp}-${random}.${ext}`;
    ...
    return NextResponse.json(
      { error: "Lỗi khi upload video lên R2" },
      { status: 500 }
    );
  }
}
